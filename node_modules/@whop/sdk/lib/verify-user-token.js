...
function getRemoteJwks(url) {
    ...
    if (existing)
        return existing;
    existing = (0, jose_1.createRemoteJWKSet)(new URL(url), {
...
async function internalVerifyUserToken(tokenOrHeadersOrRequest, options) {
    try {
        ...
        if (options.publicKey) {
            const key = await (0, jose_1.importJWK)(JSON.parse(options.publicKey), 'ES256').catch(() => {
                throw new Error('Invalid public key provided to verifyUserToken');
            });
            token = await (0, jose_1.jwtVerify)(tokenString, key, verifyOptions).catch(() => {
                throw new Error('Invalid user token provided to verifyUserToken');
            });
        }
        else {
            const jwks = getRemoteJwks(options.jwksUrl ?? DEFAULT_JWKS_URL);
            token = await (0, jose_1.jwtVerify)(tokenString, jwks, verifyOptions).catch(() => {
                throw new Error('Invalid user token provided to verifyUserToken');
            });
        }
